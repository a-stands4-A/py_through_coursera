...
элементы,а после этого удаляет последний элемент списка при помощи
метода pop().
...
Метод pop(index) у списка позволяет решить эту задачу в одну команду.
Но в этой задаче мы хотим, чтобы метод pop(index) не использовался:
реализуйте удаление элемента самостоятельно.
"""
"""
Тест 1
Входные данные:
